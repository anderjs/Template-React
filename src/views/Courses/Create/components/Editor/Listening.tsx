...
> & {
  ...
  status: boolean;
  correct: number;
  onCompile: (code: ListeningAbstraction) => void;
};

export type ListeningAbstraction = Omit<ListeningEditorProps, "onCompile">;

const ListeningEditor: React.FC<ListeningEditorProps> = ({
  index,
  status,
  answers,
  correct,
   ...
   */
  const { control, reset, getValues } = useForm<ListeningEditorProps>({
    defaultValues: {
      correct,
      question,
    },
  ...
  const draft = useForm<ListeningEditorProps>({});
   ...
   */
  const { message, compile } = useCompile<ListeningAbstraction>({
    onSuccess: (code) => {
      reset({
    ...
    <>
      ...
      <Fade delay={0.1}>
        <Card title="Selección Simple" header={TemplateClose}>
          <p className="p-0 text-gray-400">
            En Listening podrás elegir una pregunta, pistas de audio, con una
            cantidad específicas de respuestas. Al momento de finalizar el
            workflow, harás click en la opción de "Insertar" y podrás mover el
            ejercicio en la dirección que gustes.
          </p>
          <MarginY />
...
export default React.memo(ListeningEditor);
